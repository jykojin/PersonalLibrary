...
struct StatisticsView: View {
    ...
    private func computeStats() -> CachedStatistics {
        ...
        // 月度完读准备
        var monthlyMap: [String: Int] = [:]  // "YYYY-MM" -> count
        var monthlyAddedMap: [String: Int] = [:]  // 月度入库
    ...
    private var overviewSection: some View {
        LazyVGrid(columns: [
            ...
            StatCard(title: "总藏书", value: "\(totalBooks)", icon: "books.vertical", color: .blue)
            StatCard(title: "已读完", value: "\(booksRead)", icon: "checkmark.circle", color: .green)
            StatCard(title: "在读中", value: "\(booksReading)", icon: "book.fill", color: .orange)
            StatCard(title: "总页数", value: "\(totalPagesRead)", icon: "doc.text", color: .purple)
        }
        .padding(.horizontal)
    }
    ...
    private var bookTypeSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader(title: "书籍类型", icon: "square.stack.3d.up")
    ...
    private var readingStatusSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader(title: "阅读状态", icon: "list.bullet")
    ...
    private var typeDetailSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader(title: "分类型阅读状态", icon: "rectangle.split.3x1")

            VStack(spacing: 16) {
                if stats.paperStats.total > 0 {
                    typeStatusCard(title: "纸质书", stats: stats.paperStats, color: .brown)
                }
                if stats.ebookStats.total > 0 {
                    typeStatusCard(title: "电子书", stats: stats.ebookStats, color: .blue)
                }
                if stats.audiobookStats.total > 0 {
                    typeStatusCard(title: "有声书", stats: stats.audiobookStats, color: .purple)
                }
            }
            .padding(.horizontal)
        }
    ...
    private func typeStatusCard(title: String, stats: TypeStatusStats, color: Color) -> some View {
    ...
    private var bookshelfSection: some View {
        Group {
            if !stats.bookshelfStats.isEmpty {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "书架统计", icon: "books.vertical")
    ...
    private var addSourceSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            SectionHeader(title: "加入方式", icon: "arrow.down.to.line")
    ...
    private var monthlyAddedChartSection: some View {
        Group {
            if stats.monthlyAdded.contains(where: { $0.count > 0 }) {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "月度入库", icon: "chart.bar.xaxis.ascending")
    ...
    private var readingTimeSection: some View {
        Group {
            if totalMinutesRead > 0 || totalPagesRead > 0 {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "阅读数据", icon: "clock")

                    LazyVGrid(columns: [
                        GridItem(.flexible()),
                        GridItem(.flexible())
                    ], spacing: 12) {
                        InfoCard(title: "累计时长", value: formatTotalTime(totalMinutesRead), icon: "timer")
                        InfoCard(title: "累计页数", value: "\(totalPagesRead) 页", icon: "doc.text")
                        InfoCard(title: "阅读天数", value: "\(uniqueReadingDays) 天", icon: "calendar")
                        InfoCard(title: "日均页数", value: dailyPages, icon: "chart.line.uptrend.xyaxis")
                    }
                    .padding(.horizontal)
                }
            }
    ...
    private var monthlyChartSection: some View {
        Group {
            if monthlyFinished.contains(where: { $0.count > 0 }) {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "月度完读", icon: "chart.bar")

                    Chart {
                        ForEach(Array(monthlyFinished.enumerated()), id: \.offset) { index, item in
                            BarMark(
                                x: .value("月份", item.month),
                                y: .value("数量", item.count)
                            )
    ...
    private var yearlyChartSection: some View {
        Group {
            if yearlyAdded.count > 1 {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "年度入库", icon: "chart.bar.xaxis")

                    Chart {
                        ForEach(Array(yearlyAdded.enumerated()), id: \.offset) { index, item in
                            BarMark(
    ...
    private var ratingSection: some View {
        Group {
            if ratedBooksTotal > 0 {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "评分分布", icon: "star")
    ...
    private var tagRankingSection: some View {
        Group {
            if !topTags.isEmpty {
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(title: "标签 Top 10", icon: "tag")
