...
> 最后更新：v0.68（2026-09-25），包含智能补全升级与出版日期修复。注：下方第 6 节里程碑沿用旧的开发编号（tag 序列曾重排，见 commit 7f7183e），与实际 tag 号不对应，仅作功能演进参考。
...
| 出版日期规范化 | `PublicationDateParser`, `ExcelImportExportService`, `PublicationDateMigration` | 补全、表单和 Excel 共用 UTC 公历解析；支持常见中英文/分隔符格式，启动时幂等修复可确定还原的旧 Excel 异常非空日期 |
...
- **v0.68（2026-09-25）**：发布统一普通/AI 智能补全、AI 配置与微信读书 AI简介；包含 3000 字上限、后台连续性、豆瓣跨版本译者、AI JSON/身份校验和多格式出版日期及历史迁移修复。对应实现提交 `76d41e6`、`84daed6`、`e6bf307`；需求、设计、实现与验证记录见 `docs/superpowers/specs/2026-09-19-ai-enrichment-upgrade-design.md` 和对应实施计划。

- **v0.68 开发记录（2026-09-20）**：智能补全升级为统一原子能力；普通来源固定为豆瓣 → Goodreads → Open Library，新增豆瓣译者、AI 设置/模型列表/证据闸门、AI简介、独立及批量 AI 按钮，并接入微信读书自动同步。批量保持顺序执行；普通来源每本间隔 2 秒，AI 批量不另加固定等待。
- **v0.68 开发记录（2026-09-21）**：修复 qwen3.7-plus 深度思考挤占输出空间导致事实与 AI简介残缺 JSON；新增结束原因识别、分档预算、-1005 重试，并取消逐段字数死限。该阶段曾使用 900–1200 字闸门，随后已由下方“3000 字与 Unicode 资源修订”取代。模拟器真实生成《大一统的制度密码》约 128 秒通过。
- **v0.68 开发记录（2026-09-21 安全收口）**：归档微信书不再进入自动 AI/划线队列，并在耗时调用后复查归档状态以关闭 TOCTOU 窗口；普通元数据响应改为 5 MB 流式上限，重定向仅允许同主机、同端口 HTTPS；豆瓣书名回退最多检查 5 个候选；AI简介在高成本重复窗口分析前先执行资源上限（当时为 1200 字，现为 3000 字）；AI 阶段截止改为不等待底层取消完成的真正硬截止，事实检索的 4096 → 8192 重试共用一个 60 秒总预算。
- **v0.68 开发记录（2026-09-21 AI简介语义修订）**：综合情况、主题与写法、阅读体验、推荐与延伸仅作为生成方向，不再强制每段命中特定关键词或覆盖全部要点；相似书比较与扩展阅读改为可选，并移除 `comparison_books` 结构及其书名、理由和独立来源闸门，避免辅助信息不完整时拒绝整篇有效简介。
- **v0.68 开发记录（2026-09-21 AI简介长度修订）**：移除 900 字最低验收门槛；1000–1100 字仅作为生成建议，结构完整、来源有效的精炼正文直接采用。该阶段的 1200 字上限已由下一条修订放宽到 3000 字。
- **v0.68 开发记录（2026-09-21 AI简介 3000 字与 Unicode 资源修订）**：按产品要求把 AI简介安全上限放宽为 3000 个非空白显示字符，1000–1100 字建议目标不变；同时增加 Unicode scalar、UTF-8 字节和 256 KB 最终请求体上限，阻断组合附加符绕过而不改变正常中文字数语义。
- **v0.68 开发记录（2026-09-22 后台连续性）**：修复添加/编辑页 `onDisappear` 在切后台时误取消 AI 补全；添加、编辑和批量补全统一申请短时后台额度。补全中禁止下拉关闭和保存，明确取消仍有效；552 个单元/集成测试与 3 个 UI 测试通过，模拟器真实 Home→返回场景通过，并已覆盖安装到“多洛霍夫”iPhone 14 Plus。
- **v0.68 开发记录（2026-09-22 补全结果语义与出版日期）**：《人生问答》事实响应为完整 `status:error` 时不再伪装成整本书“未找到”，而是在同一阶段预算内重试并最终按协议拒绝；提示词明确已核实书籍但字段缺失时返回 `status:ok` 与空 `fields`。出版日期解析兼容豆瓣常见的 `yyyy-M` / `yyyy-M-d`，修复《大一统的制度密码》`2026-8` 以及同类日期被丢弃。
- **v0.68 开发记录（2026-09-22 AI简介身份与空字段审计）**：《人生问答》的完整副标题不再被误判为另一本书；有效输入 ISBN 现在要求 AI简介响应返回等价 `matched_isbn`，同时保留作者匹配并只放宽明确冒号形成的副标题。真机只读审计确认空页数/定价多数来自上游缺值或尚未运行普通补全，空出版日期中另有旧版非补零日期解析留下的历史数据；详见实施计划 6.9–6.10。
- **v0.68 开发记录（2026-09-23 出版日期解析与历史迁移）**：`PublicationDateParser` 统一兼容纯年、非补零年月日、点号/斜杠/中文/英文月份、Excel `yyyy.0` 和全角点号/斜杠，Excel 导入改为复用同一路径。启动迁移仅修复能确定还原的 5–6 位异常年份（如 `20239 → 2023-09-01`），无法恢复已为 `nil` 的历史空值。最终 570 个单元/集成测试（80 suites）和 3 个 UI 测试通过，模拟器与 iPhone 14 Plus 构建成功，并已覆盖安装到真机。
...
## 9. 修复追踪表（v0.59–v0.68，2026-08-09 ~ 09-25）

记录各版本集中排查的问题。每行给出 **症状 → 版本 → commit → 根因 → 防护测试**，
...
| 10 | AI简介被字数或内容方向强制校验拒绝，切到后台后补全停止 | v0.68 | `76d41e6` | 旧合同把写作建议当必填闸门，页面消失又误触发取消；现采用 3000 字上限并持有短时后台租约 | `AIIntroductionContractTests`、`EnrichmentEntryIntegrationTests` | 实施计划 6.2、6.5 |
