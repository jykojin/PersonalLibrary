...
- **未发布（2026-09-22 补全结果语义与出版日期）**：《人生问答》事实响应为完整 `status:error` 时不再伪装成整本书“未找到”，而是在同一阶段预算内重试并最终按协议拒绝；提示词明确已核实书籍但字段缺失时返回 `status:ok` 与空 `fields`。出版日期解析兼容豆瓣常见的 `yyyy-M` / `yyyy-M-d`，修复《大一统的制度密码》`2026-8` 以及同类日期被丢弃。
- **未发布（2026-09-22 AI简介身份与空字段审计）**：《人生问答》的完整副标题不再被误判为另一本书；有效输入 ISBN 现在要求 AI简介响应返回等价 `matched_isbn`，同时保留作者匹配并只放宽明确冒号形成的副标题。真机只读审计确认空页数/定价多数来自上游缺值或尚未运行普通补全，空出版日期中另有旧版非补零日期解析留下的历史数据；详见实施计划 6.9–6.10。
